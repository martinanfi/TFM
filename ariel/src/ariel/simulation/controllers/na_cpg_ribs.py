...
# Global constants
E = 1e-9
# Steering exponent n from Luo et al. (2022) Eq. (4): g(theta) = ((pi-|theta|)/pi)**n
STEERING_EXPONENT = 7
...
class NaCPG(nn.Module):
    ...
    def __init__(
        ...
        base_params,
        laterality: dict[int, LateralSide] | None = None,
        alpha: float = 0.1,
        dt: float = 0.01,
        ...
        *,
        ...
        laterality : dict[int, LateralSide] | None, optional
            Mapping from CPG index to LateralSide (LEFT/RIGHT/MIDDLE). Used by the
            steering policy in ``forward`` when ``use_angle=True``. By default None.
    ...
    @staticmethod
    def _steering_gain(alpha_angle: float, n: int = STEERING_EXPONENT) -> float:
        """g(theta) = ((pi - |theta|) / pi) ** n, Luo et al. (2022) Eq. (4)."""
        abs_alpha = abs(float(alpha_angle))
        return ((torch.pi - abs_alpha) / torch.pi) ** n
    ...
    def forward(
        ...
        ----------
        alpha_angle : float
            Heading error in radians, wrapped to [-pi, pi]. Negative means the
            target is on the left, positive means on the right.
        use_angle : bool
            If True and ``self.laterality`` is set, apply the steering policy
            from Luo et al. (2022) Eq. (5-6): attenuate LEFT joints when
            alpha_angle < 0 and RIGHT joints when alpha_angle >= 0.
        ...
        with torch.inference_mode():
            ...
            # Steering attenuation factor (constant across joints this step)
            scaling_factor = self._steering_gain(alpha_angle)
            ...
            for i, (xi, yi) in enumerate(self.xy):
                ...
                # Paper steering policy (Luo et al. 2022, Eq. 5-6)
                if use_angle and self.laterality is not None:
                    side = self.laterality.get(i)
                    if side == LateralSide.LEFT and alpha_angle < 0:
                        angles[i] = scaling_factor * angles[i]
                    elif side == LateralSide.RIGHT and alpha_angle >= 0:
                        angles[i] = scaling_factor * angles[i]
...
def main() -> None:
    ...
    na_cpg_mat = NaCPG(adj_dict, base_params=None, angle_tracking=True)
